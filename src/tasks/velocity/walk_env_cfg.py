...
from .mdp import curriculums as custom_curriculums
...
def unitree_g1_walk_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    cfg.actions["joint_pos"].scale = 0.32

    # 2) 命令分布：缩短重采样时间，让策略真正学习加减速和刹停过渡。
    twist_cmd = cfg.commands["twist"]
    twist_cmd.resampling_time_range = (2.4, 3.6)
    twist_cmd.rel_standing_envs = 0.36
    twist_cmd.rel_heading_envs = 0.24
    twist_cmd.heading_control_stiffness = 0.72

    # 比上一版放开后退和角速度，否则会学成“慢转 + 姿态补偿位移”。
    twist_cmd.ranges.lin_vel_x = (0.00, 0.60)
    twist_cmd.ranges.lin_vel_y = (-0.12, 0.12)
    twist_cmd.ranges.ang_vel_z = (-0.62, 0.62)
    ...
    for group_name in ("policy", "actor"):
        ...
        cfg.observations[group_name].history_length = 4

    # 4) 奖励：转向和刹停分开处理。
    #    - 角速度跟踪更强，减少“转不起来”。
    #    - 姿态/角动量约束适中，减少速度突变后的上身晃动。
    #    - 动作平滑不要太重，否则转向响应会继续慢。
    _set_reward_weight(cfg, "track_linear_velocity", 1.20)
    _set_reward_weight(cfg, "track_angular_velocity", 1.28)
    _set_reward_param(cfg, "track_angular_velocity", "std", 0.40)

    _set_reward_weight(cfg, "flat_orientation_l2", -10.5)
    _set_reward_weight(cfg, "body_orientation_l2", -0.85)
    _set_reward_weight(cfg, "body_ang_vel", -0.085)
    _set_reward_weight(cfg, "angular_momentum", -0.038)

    _set_reward_weight(cfg, "action_rate_l2", -0.032)
    _set_reward_weight(cfg, "joint_acc_l2", -3.5e-7)
    _set_reward_weight(cfg, "soft_landing", -8.0e-3)

    # 后退脚尖/脚跟更容易蹭地，略提高目标脚高，但脚滑惩罚仍采用渐进增强。
    _set_reward_weight(cfg, "foot_clearance", -1.34)
    _set_reward_param(cfg, "foot_clearance", "target_height", 0.118)
    _set_reward_param(cfg, "foot_clearance", "command_threshold", 0.05)

    _set_reward_weight(cfg, "foot_slip", -1.78)
    _set_reward_param(cfg, "foot_slip", "command_threshold", 0.02)

    if "foot_gait" in cfg.rewards:
        cfg.rewards["foot_gait"].weight = 0.72
        cfg.rewards["foot_gait"].params["command_threshold"] = 0.05
        cfg.rewards["foot_gait"].params["threshold"] = 0.52
    ...
    cfg.curriculum["foot_slip_weight"] = CurriculumTermCfg(
        ...
        params={
            ...
            "weight_stages": [
                {"step": 0, "weight": -1.45},
                {"step": 3000 * 24, "weight": -1.58},
                {"step": 12000 * 24, "weight": -1.72},
                {"step": 22000 * 24, "weight": -1.85},
            ],
        },
    )
    ...
    cfg.curriculum["track_angular_velocity_weight"] = CurriculumTermCfg(
        ...
        params={
            ...
            "weight_stages": [
                {"step": 0, "weight": 1.05},
                {"step": 3000 * 24, "weight": 1.15},
                {"step": 12000 * 24, "weight": 1.25},
                {"step": 22000 * 24, "weight": 1.32},
            ],
        },
    )
